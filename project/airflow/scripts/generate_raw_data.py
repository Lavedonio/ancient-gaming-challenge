...
class DataCreation:
    ...
    def generate_user_datapoints(self):
        ...
        self.user = User(
            self.user_id,
            self.user_name,
            registration_date=signup_datetime.date(),
        ...
        self.user_preference = UserPreference(
            ...
            updated_at=self.__generate_random_datetime(signup_datetime, datetime(2024, 12, 12))
        )
        days_since_signup = (TODAY - signup_datetime).days
        self.number_of_transactions = min(randrange(0, 10), days_since_signup)

    def generate_transactions(self):
        global TRANSACTION_COUNTER
        total_amount = 0
        last_transaction_date = self.user.registration_date
